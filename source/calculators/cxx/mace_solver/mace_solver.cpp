...
// ===================================================================
//  CalcBuffer constructor — pre-allocate all tensors
// ===================================================================
...
CalcBuffer::CalcBuffer(torch::Device dev, torch::ScalarType tp,
                       ...
                       int64_t max_atoms_per_struc,
                       int64_t max_edges_per_struc)
    : device(dev), dtype(tp), r_max(cutoff),
      max_batch_size(batch_size_in),
...
MaceSolver::MaceSolver(const std::string &model_path,
    ...
    buf_ = std::make_unique<CalcBuffer>(
        dev, tp, num_classes, z_map_cpu, r_max, batch_size);
...
// ===================================================================
//  begin_batch / push / batch_count — fill pinned CPU memory directly
// ===================================================================
...
bool MaceSolver::push(std::vector<int32_t> &&atomic_numbers,
    ...
    std::memcpy(pos_ptr + begin * 3, positions.data(),
                n * 3 * sizeof(double));
    std::memcpy(z_ptr + begin, atomic_numbers.data(),
                n * sizeof(int32_t));
...
// ===================================================================
//  flush() — build device tensors, forward, unpack, reset
// ===================================================================
...
std::vector<MaceOutput> MaceSolver::flush() {
    ...
    buf_->batch.slice(0, 0, N)
        .copy_(buf_->pinned_batch.slice(0, 0, N), is_cuda);
    buf_->positions.slice(0, 0, N)
        .copy_(buf_->pinned_positions.slice(0, 0, N), is_cuda);
    ...
    buf_->node_attrs.slice(0, 0, N)
        .scatter_(1, class_idx.unsqueeze(1).to(torch::kInt64), 1.0);
    ...
    // cell
    {
        ...
        buf_->cell.slice(0, 0, bs)
            .copy_(single.unsqueeze(0).repeat({bs, 1, 1}));
    }
    ...
    data.insert("positions",   buf_->positions.slice(0, 0, N)
    ...
    data.insert("unit_shifts", buf_->unit_shifts.slice(0, 0, buf_->total_edges));
